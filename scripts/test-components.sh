...
# Phase C/D are internal primitives. Keep their HTTP/UI mutation boundary
# closed and reject the prohibited operational surfaces at source level.
if grep -R -n -E '/api/v1/components/(preview|apply|rollback|policy)' internal/httpapi; then
	echo "Phase C/D HTTP mutation route detected" >&2
	exit 1
fi
