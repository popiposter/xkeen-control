...
const validProjection = (value) => value && validPolicy(value.policy)
  ...
  && ['editable', 'drift-detected'].includes(value.authorityState)
  && ['default', 'persisted'].includes(value.persistedSource)
  && (value.persistedReasonCode == null || safeText(value.persistedReasonCode))
...
const errorResult = (cause, apply) => {
  switch (cause.code) {
    ...
    case 'drift-detected': return { tone: 'error', title: 'Performance policy drift detected', message: apply ? 'The one-shot Apply outcome is not proven. A fresh read is required and mutation remains blocked while authority drift exists.' : 'The persisted authority no longer matches the active runtime policy. Refresh and inspect the closed drift state.', refresh: true, unproven: apply }
    ...
    case 'unavailable': return { tone: 'error', title: 'Performance policy unavailable', message: apply ? 'The server could not prove the policy authority or runtime owner. The request was not replayed; a successful fresh read is required before another mutation.' : 'The server could not prove the policy authority or runtime owner.', unproven: apply }
    default: return { tone: 'error', title: apply ? 'Apply outcome unavailable' : 'Preview unavailable', message: apply ? 'The request was not replayed. A successful fresh read is required before another mutation.' : 'No mutation request was submitted.', unproven: apply }
  }
}
...
export function usePerformancePolicyController({ csrfToken, lifecycle, performanceBusy, onUnauthorized, active }) {
  ...
  const [applyUnproven, setApplyUnproven] = useState(false)
  ...
  const applyUnprovenRef = useRef(applyUnproven)
  ...
  const loadPolicy = useCallback(async ({ force = false, rebase = true } = {}) => {
    ...
    const resolvingUnknown = applyUnprovenRef.current
    ...
    try {
      ...
      if ((rebase && !resolvingUnknown) || !draftRef.current) {
      ...
      if (resolvingUnknown) {
        applyUnprovenRef.current = false
        setApplyUnproven(false)
        setResult({ tone: 'warning', title: 'Apply outcome remains unknown', message: 'A fresh policy state was read. Compare it with the preserved draft before creating another Preview.' })
      }
      return true
    } catch (cause) {
      if (requestEpoch !== epoch.current) return false
  ...
  const requestRefresh = useCallback(() => {
    if (applyUnprovenRef.current) {
      clearPreview()
      void loadPolicy({ force: true, rebase: false })
      return
    }
    if (dirtyRef.current) {
  ...
  const previewDraft = useCallback(async () => {
    if (previewGate.current || applyGate.current || previewRef.current || mutationBlocked || !validPolicy(draftRef.current)) return
    ...
    try {
      ...
      if (requestSequence !== previewSequence.current) {
      ...
      if (requestSequence !== previewSequence.current) return
      if (cause.status === 401) onUnauthorized()
      const mapped = errorResult(cause, false)
      setResult(mapped)
      if (mapped.refresh) await loadPolicy({ force: true, rebase: false })
    } finally {
      if (requestSequence === previewSequence.current) previewGate.current = false
    }
  }, [csrfToken, loadPolicy, mutationBlocked, onUnauthorized])

  const applyPreview = useCallback(async () => {
    const current = previewRef.current
    if (!current || previewGate.current || applyGate.current || mutationBlocked) return
    ...
    try {
      ...
      setResult(mapped)
      if (mapped.unproven) {
        applyUnprovenRef.current = true
        setApplyUnproven(true)
        refreshMode = 'preserve'
    ...
    if (refreshMode && requestEpoch === epoch.current) await loadPolicy({ force: true, rebase: refreshMode === 'rebase' })
  }, [csrfToken, loadPolicy, mutationBlocked, onUnauthorized])
  ...
  return useMemo(() => ({
    projection, draft, dirty, refreshConfirmation, preview, pending, result, applyUnproven,
    lifecycleBlocked, performanceBusy, authorityDrift, mutationBlocked, updateField, requestRefresh,
    discardAndRefresh, cancelRefresh: () => setRefreshConfirmation(false), previewDraft,
    cancelPreview: clearPreview, applyPreview,
  }), [applyPreview, applyUnproven, authorityDrift, clearPreview, dirty, discardAndRefresh, draft, lifecycleBlocked, mutationBlocked, pending, performanceBusy, preview, previewDraft, projection, refreshConfirmation, requestRefresh, result, updateField])
}
