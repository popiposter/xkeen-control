...
| D.2 — component lifecycle | **Active — trial beta.2 VERIFIED BETA; #64 correction delivered; F3 delivered by PR #67; #46 A-F complete; G in source-only Draft PR #81** | Issue #4 / current `main` + live `v0.3.0-beta.2` trial | The prior Xray live pair stopped at deterministic pre-commit `candidate-rejected`; PR #65 delivers the diagnostic source correction. Issue #81 adds typed fresh installation plus recognized takeover/convergence, including sole-writer and Hybrid interception ownership, with no release/router/live qualification; G does not independently lead to a release |
