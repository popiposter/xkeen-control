...
| D.2 — component lifecycle | **Active — refreshed XKeen catalog PASS; beta.2 publication + independent host verification next** | Issue #4 / #47 / PRs #48/#52 | `v0.3.0-beta.1` remains historical verified evidence only; production router remains `v0.2.0`; a new signed beta.2 must be published and host-verified before install admission resumes |
...
PR #48 then refreshed the one immutable qualified XKeen catalog entry to the September 15 upstream automated build under completed Issue #47. PR #51 subsequently parallelized ordinary PR/main CI without changing runtime or protected Release workflow semantics. PR #52 corrected D.2 sequencing so a new beta.2 publication and independent host verification precede router install admission. Exact beta.2 `source_ref` is intentionally **not** embedded in ROADMAP: Issue #4 owns the freeze after the current repository prerequisites are complete. `v0.3.0-beta.2` is not yet published.

Issue #50 is the immediate repository prerequisite before the final beta.2 freeze. It removes two scheduler-sensitive test-only node transaction false negatives while preserving production transaction budgets, authority-gate and rollback semantics. After #50 merges and exact-main CI is green, Issue #4 will record the exact frozen source/tree and publication authorization will remain a separate operator decision.
...
**Next is #50 test-reliability closeout, then final beta.2 source freeze and separately authorized protected publication + independent public-host verification**, not router install admission. Use only the exact `source_ref` frozen in #4 after #50 and exact-main qualification; if `main` moves, that freeze expires.
