...
local typed appliance state + portable backup (#3, done / v0.2.0)
        ↓
managed XKeen / Xray / geodata lifecycle (#4, active; refreshed XKeen catalog; beta.2 publication + host verification next)
        ↓
...
| D.2 — component lifecycle | **Active — refreshed XKeen catalog PASS; beta.2 publication + independent host verification next** | Issue #4 / current main `a70f8bee…` / #47 | `v0.3.0-beta.1` remains historical verified evidence only; production router remains `v0.2.0`; a new signed beta.2 must be published and host-verified before install admission resumes |
...
## D.2 / Issue #4 — active; refreshed XKeen catalog; beta.2 publication + host verification next
...
PR #48 then refreshed the one immutable qualified XKeen catalog entry to the September 15 upstream automated build under completed Issue #47. PR #51 subsequently parallelized ordinary PR/main CI without changing runtime or Release workflow semantics. Current `main` is `a70f8beeabba7b4941a62eb39a6ab1f975ab7851`, tree `b2e381cb1b5e3736c58e391e45ac63379b37964f`; exact-main CI #128 / run `34953348958` passed all `go`, `race`, `fixtures`, `frontend` and final `qualification` jobs. `v0.3.0-beta.2` is not yet tagged or released; Issue #4 freezes this current main as the next beta.2 preparation candidate, subject to separate publication authorization.
...
**Next is protected beta.2 publication plus independent public-host verification**, not router install admission. Use exact current main `a70f8bee…`, `channel=beta`, proposed `version=0.3.0-beta.2`, and only the protected Release workflow after separate operator publication authorization. After publication, independently verify the exact seven public assets, signature, pinned key, hashes/sizes, `SHA256SUMS`, manifest tuple and compatibility before any live-router admission is considered.
